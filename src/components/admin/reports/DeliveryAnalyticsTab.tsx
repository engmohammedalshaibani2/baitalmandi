    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="glass-panel p-5 text-center">
          ...
          <p className="text-3xl font-bold text-[var(--gold)]">{data.totalDeliveryFee.toLocaleString('ar-EG')} ﷼</p>
          <p className="text-sm text-gray-400 mt-2">إجمالي رسوم التوصيل</p>
        </div>
        <div className="glass-panel p-5 text-center">
          <TrendingUp size={28} className="mx-auto mb-3 text-blue-500" />
          <p className="text-3xl font-bold text-[var(--gold)]">{data.avgDeliveryFee.toLocaleString('ar-EG')} ﷼</p>
          <p className="text-sm text-gray-400 mt-2">متوسط رسوم التوصيل</p>
        </div>
        <div className="glass-panel p-5 text-center">
          <MapPin size={28} className="mx-auto mb-3 text-amber-500" />
          <p className="text-3xl font-bold text-[var(--gold)]">{data.avgDistanceKm} كم</p>
          <p className="text-sm text-gray-400 mt-2">متوسط مسافة التوصيل</p>
        </div>
        <div className="glass-panel p-5 text-center">
          <Truck size={28} className="mx-auto mb-3 text-purple-500" />
          <p className="text-3xl font-bold text-[var(--gold)]">{data.totalOrders}</p>
          <p className="text-sm text-gray-400 mt-2">عدد الطلبات</p>
        </div>
      </div>

      {/* KPI Cards - Row 2: Fee Breakdown */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div className="glass-panel p-4 text-center">
          <p className="text-2xl font-bold text-[var(--gold)]">{data.totalBaseFee.toLocaleString('ar-EG')} ﷼</p>
          <p className="text-xs text-gray-400 mt-1">الرسوم الأساسية</p>
        </div>
        <div className="glass-panel p-4 text-center">
          <p className="text-2xl font-bold text-[var(--gold)]">{data.totalExtraFee.toLocaleString('ar-EG')} ﷼</p>
        ...
        <div className="glass-panel p-4 text-center" style={{ borderColor: data.totalWeatherFee > 0 ? 'rgba(59,130,246,0.3)' : undefined }}>
          ...
          <p className="text-2xl font-bold text-[var(--gold)]">{data.totalWeatherFee.toLocaleString('ar-EG')} ﷼</p>
        ...
        <div className="glass-panel p-4 text-center" style={{ borderColor: data.totalPeakFee > 0 ? 'rgba(245,158,11,0.3)' : undefined }}>
          ...
          <p className="text-2xl font-bold text-[var(--gold)]">{data.totalPeakFee.toLocaleString('ar-EG')} ﷼</p>
            ...
            <tbody>
              {data.feeByDay.map(row => (
                <tr key={row.day} className="border-b border-[var(--border)] hover:bg-[var(--glass-bg)]">
                  ...
                  <td className="py-2.5 px-3">{row.orders}</td>
                  <td className="py-2.5 px-3 font-bold text-[var(--gold)]">{row.totalFee.toLocaleString('ar-EG')} ﷼</td>
                  <td className="py-2.5 px-3">{row.avgFee.toLocaleString('ar-EG')} ﷼</td>
                </tr>
              ))}
            </tbody>
          </table>
